...
const PriceAndStock = ({ items = [], inventory }) => {
  return (
    <div>
      <div className="py-5">
        <p>Price & Stock</p>
        <div className="border rounded-xl mt-3">
          ...
          <div className="divide-y">
            {(inventory?.length > 0 ? inventory : items).map((item, i) => (
              <div key={i} className="p-3 flex w-full gap-2">
                <p className="w-2/12 font-semibold">{item.size}</p>
                <div className={"w-3/12"}>
                  <TextInput
                    className={"w-full"}
                    name={`inventory_qty_${i}`}
